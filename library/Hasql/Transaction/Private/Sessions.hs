...
inRetryingTransaction level mode isRetrying session =
  fix $ \retry -> do
    attemptRes <- tryTransaction level mode isRetrying session
    case attemptRes of
      Just a -> return a
      Nothing -> retry

tryTransaction :: IsolationLevel -> Mode -> Bool -> Session (a, Bool) -> Session (Maybe a)
tryTransaction level mode isRetrying body = do
  ...
  bodyRes <- catchError (fmap Just body) $ \error -> do
    ...
    handleTransactionError error isRetrying $ return Nothing
...
handleTransactionError error isRetrying onTransactionError = case error of
  QueryError _ _ clientError -> onCommandError clientError
  PipelineError clientError -> onCommandError clientError
  where
    retryOrThrow = if isRetrying then onTransactionError else throwError error
    onCommandError = \case
